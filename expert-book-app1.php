...
<?php 
		...
		$appArray = $conn->execute_sql("select", array("eap_id", "eap_date"), "e_appointments", "eap_date >= ? AND eap_date <= ? order by eap_date", array("s1" => $newDate . " 00:00:00", "s2" => $newDate . " 11:59:59"));
				...
				<div class="columnBorder">
					...
					<div class="col-md-<?php echo $active[0] ?> app-box <?php echo $active[1] ?>-taken midd">
						<?php if ($active[1] == "active") { ?>
                        	...
                        	<a href="#" title="Appointment Information" class="show-overlay" id="booking:<?php echo $active[2]; ?>" value="<?php echo str_pad($i, 2, '0', STR_PAD_LEFT) . ":" . str_pad($j, 2, '0', STR_PAD_LEFT) . ":00" ?>" title="Eg, Egg and Eggy"><?php } ?><?php echo str_pad($i, 2, '0', STR_PAD_LEFT) . ":" . str_pad($j, 2, '0', STR_PAD_LEFT) ?>
                        <?php if ($active[1] == "active") { ?>
