...
//$findRequestUpdates = $conn->execute_sql("select", array("mu_id", "mu_subject", "ea_forename", "ea_surname", "mu_datesent", "ut_type"), "m_updates JOIN e_accounts ON mu_sender_id = ea_id JOIN u_types ON mu_type = ut_id", "mu_recipient_id=? AND mu_type=3 ORDER BY mu_datesent DESC", array("i" => $_SESSION['CME_USER']['login_id']));
	...
	foreach($findRequestUpdates as $header => $row) {
		...
		$firstName = $findRequestUpdates[$header]['ea_forename'];

		$lastName = $findRequestUpdates[$header]['ea_surname'];
		
		$timeSent = substr($findRequestUpdates[$header]['mu_datesent'], 11, -3);

		$dateSent = substr($findRequestUpdates[$header]['mu_datesent'], 0 , -9);
		
?>

	<a class="btn btn-primary col-md-12 badgefix mb10 subjectBox" name="<?php echo $findRequestUpdates[$header]['mu_id']; ?>">
			<p><i class="fa fa-envelope-o"></i><b><?php echo " " . $findRequestUpdates[$header]['mu_subject']; ?></b></p>
