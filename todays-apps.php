...
<?php
	...
	echo "<h4>Available Appointments On: "  . $todaydate . "</h4>";
	echo "</div>";
...
<?php 
		...
		$appArray = $conn->execute_sql("select", array("eap_id", "eap_date", "eap_timeslot"), "e_appointments", "eap_date >= ? AND eap_date <= ? AND eap_ea_id =? order by eap_date", array("s1" => $todaydate . " 00:00:00", "s2" => $todaydate . " 23:59:59", "i" => $_SESSION['CME_USER']['login_id']));
