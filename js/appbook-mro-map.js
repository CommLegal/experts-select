...
$(document).ready(function initialise(){
	...
	if($('#request_search').length) {
		...
		$('#request_search').submit(function(){
			// get address
			var address = $('#vbook-postcode').val();
	...
	if($('#map_canvas').length) {
		...
		if($('#vbook-latitude').length) {
			val = $('#vbook-latitude').val()*1;
			if(val != '' && !isNaN(val)) {
				lat = val;
				display_marker = true;
			}
		}
		if($('#vbook-longitude').length) {
			val = $('#vbook-longitude').val()*1;
			if(val != '' && !isNaN(val)) {
				lng = val;
			}
		}
	...
	if($('#vbook-postcode').length) {
		
		// when 'unclick' postcode box
		$('#vbook-postcode').blur(function(){
	...
	if(address != '') {
		...
		geocoder.geocode( {'address':address,'region':region}, function(results, status) {
			...
			if(status == google.maps.GeocoderStatus.OK) {
				...
				$('#vbook-latitude').val( results[0].geometry.location.lat() );
				$('#vbook-longitude').val( results[0].geometry.location.lng() );
